 ...
 * @returns
 */
export const analayse = (vocab, response) => {
    // TODO: Refactor: Update to newest verion of axios api
    const $ = cheerio.load(response.data);
    const title = $('div.di-title:first>span>span').text();
    vocab.title = title;
    const pos = $('div.posgram.dpos-g.hdib.lmr-5:first span.pos.dpos').text();
    const dpos = $('div.posgram.dpos-g.hdib.lmr-5:first span.gram.dgram').text();
    vocab.pos = pos;
    vocab.dpos = dpos;
    ...
    $('div.def-block.ddef_block').each((i, elem) => {
        ...
        const subList = $(elem).find('div.def-body.ddef_b div.examp.dexamp');
        if (subList.length !== 0) {
            subList.each((j, item) => {
